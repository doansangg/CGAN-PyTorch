...
class DiscriminatorForMNIST(nn.Module):
    ...
    def __init__(self, image_size: int = 224, channels: int = 3, num_classes: int = 3) -> None:
        super(DiscriminatorForMNIST, self).__init__()

        self.label_embedding = nn.Embedding(10, num_classes)
...
def discriminator_for_mnist(image_size: int = 224, channels: int = 3) -> DiscriminatorForMNIST:
